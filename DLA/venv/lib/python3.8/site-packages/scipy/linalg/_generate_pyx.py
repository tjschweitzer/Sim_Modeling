...
def generate_blas_pyx(func_sigs, sub_sigs, all_sigs, header_name):
    funcs = " .join(pyx_decl_func(*(s+(header_name,))) for s in func_sigs)
    subs = "  + " .join(pyx_decl_sub(*(s[::2]+(header_name,)))
                            for s in sub_sigs)
...
def generate_fortran(func_sigs):
    return " .join(fort_subroutine_wrapper(*sig) for sig in func_sigs)
...
def generate_c_header(func_sigs, sub_sigs, all_sigs, lib_name):
    ...
    subs = "  + "".join(c_sub_decl(*sig) for sig in sub_sigs)
    if lib_name == 'LAPACK':
